...
# ]
STANDARD_COLORS = [
   'OliveDrab', 'Orange', 'SkyBlue','White',
]
...
#  box_to_id_class_map= collections.defaultdict(list)##########
...
#            box_to_id_class_map[box].append(display_str)##########
        if not skip_scores:
          if not display_str:
            display_str = '{}%'.format(int(100*scores[i]))#信心值百分比
          else:
            display_str = 'ID:{} {}: {}%  '.format(int(i+1),display_str, int(100*scores[i]))
        box_to_display_str_map[box].append(display_str)
        if agnostic_mode:
          box_to_color_map[box] = '#5500DD'
        else:
             #box_to_color_map[box] ='#5500DD'
...
#    if 'cobia' in box_to_id_class_map[box]:
