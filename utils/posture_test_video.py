    ...
    for part in fish:
        if part != main_box_name and part != 'posture_vector':
            if len(fish[part]) != 0:
                ...
                if ((part == 'mouth') and (part_degrees['eye'] != None)):
                    degrees_list.append(4000)
                    part_degrees[part] = None
...
while (video.isOpened()):
    ...
    fish_list = getCompleteFish(all_bnd)
    for fish in fish_list:
        decidePostureVector(fish, main_box_name)
    ...
    cv2.imwrite(write_path + str(int(video.get(1))) + '.jpg', temp)
    # cv2.imwrite('./rs/' + VIDEO_NAME + '_' + str(int(video.get(1))) + '.jpg', frame)
